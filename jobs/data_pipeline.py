# jobs/data_pipeline.py

from pyspark.sql.functions import col, substring, when, lit
from pyspark.sql.types import DoubleType, IntegerType
from transformations import extract_field
import config

def run_data_pipeline(spark):
    """
    Defines the data pipeline steps:
      1) Read data from S3
      2) Apply transformations
      3) Create/replace Delta table
    """

    # 1. Read data from S3
    df = (
        spark.read.text(config.S3_PATH)
        .filter(
        ...
        .select(
            ...
            *(
                col(field).cast("string").alias(field)
                for field in [
                    "weather_station_usaf_id", "weather_station_wban_id",
                    "observation_date", "observation_time",
                    "latitude_dec", "longitude_dec", "elevation",
                    "identifier", "snow_depth", "snow_condition_code",
                    "snow_quality_code", "equiv_water_depth",
                    "equiv_water_condition_code", "equiv_water_quality_code",
                    "greatest_snow_depth", "greatest_condition_code",
                    "greatest_dates_of_occurrence", "greatest_quality_code",
                    "al1_accumulation_period_quantity", "al1_accumulation_depth",
                    "al1_accumulation_condition_code", "al1_accumulation_quality_code",
                    "al2_accumulation_period_quantity", "al2_accumulation_depth",
                    "al2_accumulation_condition_code", "al2_accumulation_quality_code",
                    "al3_accumulation_period_quantity", "al3_accumulation_depth",
                    "al3_accumulation_condition_code", "al3_accumulation_quality_code",
                    "al4_accumulation_period_quantity", "al4_accumulation_depth",
                    "al4_accumulation_condition_code", "al4_accumulation_quality_code",
                    "greatest_accumulation_depth", "am1_accumulation_condition_code",
                    "am1_dates_of_occurrence_1", "am1_dates_of_occurrence_2",
                    "am1_dates_of_occurrence_3", "am1_accumulation_quality_code",
                    "monthly_period_quantity", "monthly_accumulation_depth",
    ...
    # 2. Create or replace Delta table
    df.createOrReplaceTempView("df_temp")
    spark.sql(f"""
        CREATE OR REPLACE TABLE {config.DELTA_TABLE_NAME}
        USING DELTA
        AS SELECT * FROM df_temp
    """)

    # Optionally return df if you need it for further steps
    return df
